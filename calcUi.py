...
class UI(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        uic.loadUi("ziraCalculatorUi.ui", self)

        self.run = self.findChild(QPushButton, "runRound")
        self.input = self.findChild(QLineEdit, "totalBallsInput")
        self.run.clicked.connect(self.RunBtnClicked)
        # self.input.clicked.connect(self.DemoBtnClicked)
        self.show()
...
class LogPage(QtWidgets.QMainWindow):
    ...
    def newTables(self):
        ...
        cur.execute(
            "CREATE table ZiraLog (EntryId INTEGER PRIMARY KEY,BetId TEXT,BallsReturned TEXT, BetType TEXT,TimePlaced TEXT,LoosingStreak TEXT,InvestmentAmt TEXT,Won_Lost TEXT,AmtLost TEXT,Profit TEXT,TotalBalance TEXT,CurrentTime TEXT,CurrentDate TEXT,HighestStreak TEXT)")
